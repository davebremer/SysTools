...
.PARAMETER LAPS
  Gets LAPS credentials to authenticate to device
...
.EXAMPLE
  Get-LastLogonToSys -ComputerName svr01 -LAPS
  This example will return the last logon information from svr01 by obtaining the password from AD using LAPS

    Computer          : SVR02
    User              : WILIHTE\ADMIN
    SID               : S-1-5-21-582367480-1618812582-1850952788-109401
    Time              : 5/19/2017 8:49:39 PM
    CurrentlyLoggedOn : True
...
.NOTES 
  ...
  SourceURL: https://gallery.technet.microsoft.com/scriptcenter/Get-LastLogon-Determining-283f98ae
  
  Updates: dave bremer 2017-05-19 added LAPS auth
  ...
  { 
    Foreach ($Computer in $ComputerName) 
      { 
        ...
        if ($Laps) {$cred = get-LAPSCred $computer }
        ...
        Try 
          { 
            ...
            if ($cred) {
                $Win32OS = Get-WmiObject -Class Win32_OperatingSystem -ComputerName $Computer -Credential $cred
            } else {
            ...
            If ($Build -ge 6001) 
              { 
                ...
                if ($cred) {
                    $Win32User = Get-WmiObject -Class Win32_UserProfile -Filter $WQLFilter -ComputerName $Computer -Credential $cred
                } else {   
